 ...
 */
var control_pelicula_list = function(path) {
    //contexto privado

    var prefijo_div = "#pelicula_list ";
    ...
    function loadModalForm(view, place, id, action) {
        ...
        if (action == "edit") {
            ...
            $(prefijo_div + '#nombre_pelicula').focus();
        }

        //clave ajena genero
        cargaClaveAjena('#id_genero', '#id_genero_desc', 'genero')

        $(prefijo_div + '#id_genero_button').unbind('click');
        $(prefijo_div + '#id_genero_button').click(function() {
            loadForeign('genero', '#modal02', control_genero_list, callbackSearchGenero);
            function callbackSearchGenero(id) {
                $(prefijo_div + '#modal02').modal('hide');
                $(prefijo_div + '#modal02').data('modal', null);
                $(prefijo_div + '#id_genero').val($(this).attr('id'));
                cargaClaveAjena('#id_genero', '#id_genero_desc', 'genero');
                return false;
            }
            return false;
        ...
        $('#formulario').validate({
            ...
            messages: {
                nombre_pelicula: {
                    required: "Ingresa un nombre de pelicula."
                },
                anyo: {
                ...
                website: {
                    required: "Ingresa una website."
                },
                duracion: {
                    required: "Introduce una duracion para la pelicula."
                },
                puntuacion: {
                    required: "Introduce una puntuacion."
                },
                url: {
